...
export class RazorpayService {
  ...
  async openCheckout(options) {
    ...
    const defaultOptions = {
      ...
      callback_url: `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5001/api'}/payment/verify`,
      redirect: false,
    };
  ...
  async createOrder(amount, receipt) {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5001/api'}/payment/create-order`, {
        method: 'POST',
        headers: {
  ...
  async verifyPayment(paymentData) {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5001/api'}/payment/verify`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('featherfold_token')}`,
